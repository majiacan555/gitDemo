...
public  class Get{
	...
	public List<String> GetUrl(String Url,String Date , String Hour) throws SQLException {
...
//        httpGet.setConfig(requestConfig);
		...
		try {
			...
			if (closeableHttpResponse.getStatusLine().getStatusCode() == 200) {
				...
				Element masthead2 = doc2.select("span.list").first();
				System.out.println(masthead2.toString());
				Document doc3 = Jsoup.parse(masthead2.toString(),"utf-8");
				Element masthead3 = doc3.select("div").first();
				Document doc4 = Jsoup.parse(masthead3.toString(),"utf-8");
				Elements masthead4 = doc4.select("a");
				for(Element element : masthead4){
					list.add(element.attr("href").toString());
				}
				Elements masthead5 = doc2.select("p.comment_txt");
